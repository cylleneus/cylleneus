...
def find_similar(tokens, collection: Collection, n=4, slop=1, synsets=False):
    """
    ...
    for ngram in ngrams(tokens, n):
        ...
        if search.count != (0, 0, 0):  # matches, docs, corpora
            print([hlite for hlite in search.highlights])
...
if __name__ == "__main__":
    print("""
    ================================ CYLLENEUS QUOTATION FINDER ================================
    """)

    # Load target text and process, if necessary
    with codecs.open('corpus/perseus/text/virgil__aeneid__latin.json', 'r', 'utf8') as fp:
        doc = json.load(fp)

    meta = 'book-line'

    # Set the search collection
    collection = Collection()
    collection.add(Work(corpus=Corpus('perseus'), author='virgil', title='eclogues'))
