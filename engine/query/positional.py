...
class Position(engine.query.compound.CylleneusCompoundQuery):
    """Matches documents containing a list of sub-queries in adjacent
    positions.
    This object has no sanity check to prevent you from using queries in
    different fields.
...
class Position2(engine.query.compound.CylleneusCompoundQuery):
    """Matches documents containing a list of sub-queries at the same
    position.
    This object has no sanity check to prevent you from using queries in
    different fields.
    """
    ...
    def __init__(self, subqueries, slop=0, ordered=False, boost=1.0, meta=False):
        """
        :param subqueries: a list of :class:`whoosh.query.Query` objects to
            match in sequence.
        :param slop: the maximum difference in position allowed between the
    ...
    def _and_query(self):
        return engine.query.positional.Position(self.subqueries)
    ...
    def _matcher(self, subs, searcher, context):
        ...
        # spans
        context = context.set(needs_current=True)
        m = engine.query.spans.SpanAt(subs).matcher(searcher, context)
        return m
