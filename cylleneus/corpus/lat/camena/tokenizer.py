...
from cylleneus.lang.lat import jvmap, word_tokenizer, enclitics
        ...
        if kwargs.get('docix', None) == self._docix and self._cache:
            ...
            if t.mode == 'query':
                ...
                if not tokenize:
                    ...
                    punc = str.maketrans("", "", punctuation)
                    ...
                    for el in data['text'].find("text").find("body").iter():
                        if el.tag in divs:
                            ...
                            if not el.text:
                                ...
                                text = el.text if el.text else re.sub(r"b'<.+?>(.+?)</.+?>\n'", "\1", et.tostring(el))
                            subs = [r"<note>(.*?)</note>", r'<sic corr="(\w+?)">\w+?</sic>',
                                    r'<reg orig="\w+?">(\w+?)</reg>']
                            for sub in subs:
